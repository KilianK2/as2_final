...
def train_svm_with_cv(X_train, y_train, groups_train=None, cv=5, random_state=42):
    """
    Train an SVM classifier with hyperparameter tuning using cross-validation.
    ...
    # Define hyperparameter grid for tuning - focus on RBF kernel only for better performance
    param_grid = {
    ...
    # Initialize base model - using RBF kernel which usually works best for complex data
...
def evaluate_model(model, X_test, y_test, class_names=None):
    ...
    results_folder = os.path.join('results', 'svm')
    os.makedirs(results_folder, exist_ok=True)
    plt.savefig(os.path.join(results_folder, 'confusion_matrix.png'))
    plt.close()
    
    # For SVM: If linear kernel is used, we can get coefficients
    if hasattr(model, 'coef_'):
        ...
        print("\nTop 20 Most Important Features (based on SVM coefficients):")
        print(feature_importances.head(20))
        
        # Plot feature importances
        plt.figure(figsize=(12, 8))
        sns.barplot(x='Coefficient', y='Feature', data=feature_importances.head(20))
        ...
        feature_importances.to_csv(os.path.join(results_folder, 'feature_importance.csv'), index=False)
...
def main():
    ...
    # Apply normalization using StandardScaler - matching the Random Forest approach
    ...
    # Train model with hyperparameter tuning - using normalized data
    best_model, best_params = train_svm_with_cv(
    ...
    accuracy = evaluate_model(best_model, X_test_scaled, y_test, class_names)
